(function() {
    ...
    var $$action$events$$eventMe = function(objectIn){
       'use strict';
       ...
       returnObject.emit = function(eventNameIn, eventDataIn, localFlag){
           var that = this
               , functionToCall
               , eventStack = (typeof local !== 'undefined' && local) ? that.eventStore[eventNameIn] : src$es6$action$shell$$action.eventStore[eventNameIn];
       ...
       returnObject.listen = function(eventNameIn, handlerIn, scopeIn, onceIn, localFlagIn){
           ...
           eventStack = (typeof local !== 'undefined' && local) ? that.eventStore[eventNameIn] : src$es6$action$shell$$action.eventStore[eventNameIn];
           newEvent = (typeof local !== 'undefined' && local) ? that : src$es6$action$shell$$action;
       ...
       returnObject.listenOnce = function(eventNameIn, handlerIn, scopeIn, localFlagIn){
           ...
           if(typeof localFlag !== 'undefined' && localFlag){
               ...
               eventStack = src$es6$action$shell$$action.eventStore[eventName];
               newEvent = src$es6$action$shell$$action;
           }
       ...
       returnObject.silence = function(eventNameIn, handlerIn, onceIn, scopeIn, localFlagIn){
           ...
           store = (typeof localFlag === 'undefined' || !localFlag) ? src$es6$action$shell$$action : that;
       ...
       returnObject.listen('system:trace', function(emitterIDIn){
           ...
           if(that.emitterId === emitterIDIn){
               ...
               src$es6$action$shell$$action.traced = that;
           }
       }, returnObject);
    ...
    var $$action$utils$$Error =  function(typeIn, messageIn, objectIn, errorObjectIn){
            'use strict';
        ...
        //a clone function
        , $$action$utils$$clone =  function(objectIn, cloneMe){
            'use strict';
    ...
    var $$action$model$$modelMe = function(objectIn){
            'use strict';

            //this is the module for creating a data model object
            var that = this
                , newModel = $$action$events$$eventMe({})
                , attributes = {}
                , changes = [];
                ...
                if(typeof attributeName === 'object'){
                        ...
                        if(attributeName.hasOwnProperty(key)){
                            ...
                            if(key !== 'destroy' && key !== 'fetch' && key !== 'save' && typeof attributeName[key] !== 'function'){
                                if(typeof attributeValue === 'object'){
                                    ...
                                    $$action$utils$$clone(attributes[attributeName], attributeName[key]);
                                }else{
                    ...
                    if(attributeName !== 'destroy' && attributeName !== 'fetch' && attributeName !== 'save'){
                        if(typeof attributeValue === 'object'){
                            ...
                            $$action$utils$$clone(attributes[attributeName], attributeValue);
                        }else{
            ...
            newModel.fetch = function(setVariableName, successFunction, errorFunction, flushCache){
                ...
                if(typeof requestUrl !== 'undefined'){
                    ...
                    that.emit('global:error', new $$action$utils$$Error('http', 'No URL defined', that));
            ...
            newModel.ajaxGet = function(setVariableName, successFunction){
                ...
                oReq.onload = function(){
                            ...
                            if(this.status === 200 || this.status === 302){
                                ...
                                that.emit('global:error', new $$action$utils$$Error('http', 'Error in request', that));
                            }
                        };

                oReq.onerror = function(xhr, errorType, error){
                            that.emit('global:error', new $$action$utils$$Error('http', 'Error in request type: ' + errorType, that, error));
                        };
            ...
            newModel.save = function(){
                ...
                if(typeof requestUrl !== 'undefined'){
                    ...
                    action.emit('global:error', new $$action$utils$$Error('http', 'No URL defined', that));
                }
            }
    ...
    var $$action$route$$routeMe = function(objectIn){
            'use strict';
    ...
    var $$action$view$$viewMe = function(objectIn){
        'use strict';
        
        var that = this
            , newView = $$action$events$$eventMe(objectIn);
    ...
    var src$es6$action$shell$$action = {
        eventStore: {}
        , eventMe: $$action$events$$eventMe
        , routeMe: $$action$route$$routeMe
        , viewMe: $$action$view$$viewMe
        , modelMe: $$action$model$$modelMe
        , clone: $$action$utils$$clone
        , Error: $$action$utils$$Error
    };

    src$es6$action$shell$$action = $$action$events$$eventMe(src$es6$action$shell$$action);

    src$es6$action$shell$$action.listen('template:get', function(templateID){
        'use strict';

        src$es6$action$shell$$action.emit('template:set:' + templateID, src$es6$action$shell$$action.templates[templateID]);
    });

    src$es6$action$shell$$action.listen('global:error', function(errorIn) {
        'use strict';
    ...
    this.action = src$es6$action$shell$$action;
}).call(this);
